...
static int smi2021_initialize(struct smi2021 *smi2021)
{
	...
	for (i = 0; i < ARRAY_SIZE(init); i++) {
		//rc = smi2021_set_reg(smi2021, 0x4a, init[i][0], init[i][1]);
...
//			{ 0x11, 0x0c },
	...
	for (i = 0; i < ARRAY_SIZE(init_DBG); i++) {
		rc = smi2021_set_reg(smi2021, 0x4a, init_DBG[i][0], init_DBG[i][1]);
		if (rc < 0)
			return rc;
	}
	*/
